...
const CenteredOverlayCard = ({ title, imageURL, message, location, dateDetails, primaryButton, secondaryButton, background}) => (
    <>
        <div className="flex flex-col justify-between h-screen w-full bg-black">
            <div className='flex flex-col'>
                {secondaryButton}
                <p className="text-slate-100 text-4xl text-center font-extrabold font-dancingScript tracking-wide mt-10">
                    {"You're invited to"}
                </p>
                <p className="text-slate-100 text-4xl mt-5 uppercase text-center font-playfairDisplay">
                    {title}
                </p>
            </div>
            <picture>
            ...
            <div className="flex flex-col shadow items-stretch px-2">
                { message !== undefined && <p className="text-slate-100 text-lg font-semibold text-left font-montserrat px-2">{message}</p>}
                <CardDetails
                    text={dateDetails}
                    icon={<ClockIcon className="mr-1 h-5 w-5 text-slate-900" />}
                />
...
const InvitationCard = ({ title, imageURL, message, location, startDate, endDate, primaryButton, secondaryButton, isPortraitImage }) => {
    ...
    return (
        (isPortraitImage) ?
            ...
            <CenteredOverlayCard title={title} imageURL={imageURL} message={message} location={location} dateDetails={dateDetails} primaryButton={primaryButton} secondaryButton={secondaryButton} />
    )
}
